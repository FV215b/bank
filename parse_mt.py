...
def handlexml(request):
    ...
    try:
        ...
        error.text = "Cannot open database"
        return tostring(error)
    cur = db.cursor()
    try:
        tree = ET.ElementTree(XML(request))
    except:
        error.text = "Input cannot parse to XML"
        return tostring(error)
    root = tree.getroot()
    top = Element('results')
    ...
    for child in root:
        ...
        if child.tag == "create":
            handle_create(child, top, cur, db)
        elif child.tag == "transfer":
            handle_transfer(child, top, cur, db)
        elif child.tag == "balance":
            handle_balance(child, top, cur)
        elif child.tag == "query":
            handle_query(child, top, cur)
    start_time = time.time()
    xml_string = tostring(top) 
    end_time = time.time()
    print("tostring -------- %s seconds --------" %(end_time - start_time))
    return xml_string
...
def handle_create(create_node, top, cur, db):# add reponse parameter to get the responding response
    ...
    if len(balances) > 1:
        ...
        return
    if len(balances) == 0:
        if create_account(account_num, 0, cur, db):
    ...
    if is_valid_float_number(balances[0].text):
        if create_account(account_num, float(balances[0].text), cur, db):
            addxml(top, 'success', create_node, 'created')
        else:
...
def create_account(account_num, money, cur, db): #need to check the db/data structure
...
def is_valid_account(account_num, cur): #check 64 bit unsigned number and check in db/data structure
...
def update_from_account_balance(account_num, amount, cur):
...
def update_to_account_balance(account_num, amount, cur):
...
def handle_transfer(transfer_node, top, cur, db):
    ...
    lock.acquire()
    if is_valid_account(tos[0].text, cur):
        if update_from_account_balance(from_account, amount, cur):
            if update_to_account_balance(to_account, amount, cur):
                db.commit()
                lock.release()
...
def handle_balance(balance_node, top, cur):
    accounts = balance_node.findall('account')
    if len(accounts) != 1:
        addxml(top, 'error', balance_node, 'Exactly one account is allowed')
        return
    if not is_valid_account(accounts[0].text, cur):
        addxml(top, 'error', balance_node, 'Account does not exist')
        return
    addxml(top, 'success', balance_node, check_balance(to_64_char(accounts[0].text), cur))

def check_balance(account_num, cur): #need to check db/data structure to query
...
def dfs(root, flag_container, cur):
    if flag_container[0]:
        return ""
    if root.tag == "equals":
        ...
        children_list = []
        for child in root:
            dfs_value = dfs(child, flag_container, cur)
            if dfs_value != "":
